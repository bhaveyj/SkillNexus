...
export default function MasterclassesPage() {
  ...
  const mySessions: RegisteredMasterclass[] = mySessionsData ?? [];
  const userRole: string | null = profileData?.role ?? null;

  const completedSessions = mySessions.filter((sessionItem) => sessionItem.isCompleted);
  ...
  const filteredCompleted = completedSessions.filter(sessionItem => {
    const matchesSearch = sessionItem.title.toLowerCase().includes(searchQuery.toLowerCase());
    const matchesCat = selectedCat === "All" || sessionItem.category === selectedCat;
  ...
  const handleSubmitQuiz = async () => {
    if (!quizSession || !quizPayload) return;

    const answers = quizPayload.questions.map((q) => ({
      questionId: q.id,
      selectedOption: quizAnswers[q.id],
    }));

    if (answers.some((a) => a.selectedOption === undefined)) {
      setToast({ message: "Please answer all questions", type: "warning" });
      return;
    }
  ...
  const reviewQuestions = quizReviewQuestions ?? quizPayload?.questions ?? [];
  const feedbackById = new Map(quizAttempt?.feedback?.map(item => [item.questionId, item]) ?? []);
  ...
  return (
    <div className="flex-1 overflow-auto">
      ...
      <Dialog open={quizOpen} onOpenChange={(open) => { if (!open) resetQuizState(); }}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto bg-[#0d0a1e] border-white/[0.08] shadow-2xl p-0">
          <div className="px-6 pt-6 pb-4 border-b border-white/[0.05]">
            <p className="text-[10px] font-black uppercase tracking-[0.18em] text-foreground/30">Masterclass Quiz</p>
            <h3 className="text-xl font-bold mt-1">{quizTitle || "Quiz"}</h3>
            <p className="text-xs text-foreground/45 mt-1">AI-generated questions with instant evaluation.</p>
          </div>
          ...
          {quizLoading ? (
            ...
            <div className="p-6 space-y-6">
              <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div>
                  <p className="text-xs text-foreground/45">Quiz Results</p>
                  <p className="text-sm text-foreground/70 mt-1">{quizAttempt.summary}</p>
                </div>
                <div className="px-4 py-3 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-center">
                  <p className="text-[10px] font-black uppercase tracking-[0.14em] text-emerald-300/70">Score</p>
                  <p className="text-2xl font-bold text-emerald-300 mt-1">{quizAttempt.score}/{quizAttempt.maxScore}</p>
                </div>
              </div>
              ...
              <div className="space-y-4">
                {reviewQuestions.map((q, idx) => {
                  ...
                  const correctOption = feedback?.correctOption ?? q.correctOption;
                  const selectedOption = feedback?.selectedOption ?? -1;
                  return (
                    <div key={q.id} className="rounded-2xl border border-white/[0.08] bg-white/[0.02] p-4">
                              ...
                              className={cn(
                                ...
                                isCorrect
                                  ...
                                  : isSelected
                                    ...
                                    : "border-white/[0.08] bg-white/[0.02] text-foreground/70",
                              )}
                            >
                              {opt}
                            </div>
                      ...
                      {feedback?.explanation && (
                        <p className="text-xs text-foreground/50 mt-3 leading-relaxed">{feedback.explanation}</p>
                      )}
                    </div>
                  );
                })}
              ...
              <div className="flex items-center justify-end gap-2">
                <Button variant="glass" onClick={resetQuizState} className="h-9 text-xs">Close</Button>
                {quizSession && (
                  <Button onClick={() => openQuiz(quizSession)} className="h-9 text-xs gap-1.5">
                    <FileQuestion size={12} /> Retake Quiz
                  </Button>
                )}
              </div>
            ...
            <div className="p-6 space-y-6">
              <div className="flex items-center justify-between">
                <div className="text-xs text-foreground/45">Answered {answeredCount}/{quizPayload.questions.length}</div>
                <div className="text-[10px] font-black uppercase tracking-[0.18em] text-foreground/30">10 Questions</div>
              </div>
                        ...
                        {q.options.map((opt, optIndex) => {
                          ...
                          return (
                            <button
                              ...
                              className={cn(
                                "w-full text-left px-3 py-2 rounded-xl border text-xs font-semibold transition-all",
                                isSelected
                                  ? "border-violet-500/40 bg-violet-500/10 text-violet-200"
                                  : "border-white/[0.08] bg-white/[0.02] text-foreground/70 hover:border-white/[0.18]",
                              )}
                            >
                              {opt}
                            </button>
              ...
              <div className="flex items-center justify-between">
                <p className="text-xs text-foreground/35">AI will evaluate and explain each answer.</p>
                <Button onClick={handleSubmitQuiz} disabled={quizSubmitting} className="h-9 text-xs gap-1.5">
                  {quizSubmitting ? <><Spinner />Submitting…</> : "Submit Quiz"}
                </Button>
              </div>
            </div>
          ) : (
            <div className="p-6">
              <p className="text-sm text-foreground/50">Unable to load quiz data.</p>
            </div>
          )}
        </DialogContent>
      </Dialog>
